...
foreach ($entry in $manifest.assets) {
    if ($entry.file -notmatch '^(HourstoneCompanion[-.A-Za-z0-9]*\.(exe|nupkg|zip)|RELEASES|releases\.win\.json|assets\.win\.json|dependencies\.cdx\.json|DEPENDENCY-NOTICES\.txt|ASSET-NOTICES\.txt|assets-manifest\.json|LICENSE\.txt|SHA256SUMS)$') { throw 'Unexpected release asset.' }
